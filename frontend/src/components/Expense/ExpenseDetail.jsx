...
const ExpenseDetail = () => {
  ...
  const {
    ...
    queryFn: () => ExpenseServices.GetExpenseList(page, limit),
    queryKey: ["get-expense-list", page, limit],
    enabled: !!page && !!limit,
    select: (d) => d?.data,
  ...
  useEffect(() => {
    ...
    if (query !== "") {
      getData = setTimeout(() => {
        ExpenseServices.SearchExpenseList(query)
          .then((response) => {
  ...
  return (
    <div className="px-8">
      <div className="flex justify-between my-4">
        ...
        <Link to="/expense/add-expense" className="">
          <Button>
            <FiPlus className="" />
            Add New
          </Button>
